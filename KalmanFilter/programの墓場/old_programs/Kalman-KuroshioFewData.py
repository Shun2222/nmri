...
def get_z(ais_cur1, ais_cur2, dtidx, default=None):
    ...
    ais_cur1_dt = kurosio_filter(ais_cur1[dtidx][0], nan_map) # 時刻0のais data
    ais_cur2_dt = kurosio_filter(ais_cur2[dtidx][0], nan_map) # 時刻0のais data
...
def get_x(jcope_n, jcope_e, dtidx, default=np.nan):
    ...
    jcope_n_dt = kurosio_filter(jcope_n[dtidx], nan_map) # 時刻0のjcope data
    jcope_e_dt = kurosio_filter(jcope_e[dtidx], nan_map) # 時刻0のjcope data
...
Q = np.eye(_2NM+1)*0.001 # システム誤差(jcope)
...
P = np.zeros(_2NM+1, _2NM+1) # 初期事後誤差共分散行列
...
with open(f'{save_dir}/saverH{year}{month:02}{day:02}00-N{_N}.pkl', 'wb') as f:
    pickle.dump(H, f)
jcor = H @ jcope
with open(f'{save_dir}/saverJCOPECur{year}{month:02}{day:02}00-N{_N}.pkl', 'wb') as f:
    pickle.dump(jcor, f)
with open(f'{save_dir}/saverXCur{year}{month:02}{day:02}00-N{_N}.pkl', 'wb') as f:
    pickle.dump(H@x, f)
...
#for day in range(23, 23+1):
    ...
    #for t in tqdm.tqdm(range(1,5)):
        ...
        #is_success = True
        #try:
        #    SI = np.linalg.inv(S) #TODO 普通の疑似逆行列は左からかける前提，今回は右からだが，影響はないか
        #    inv = 'inv'
        #except:
        #    pm.printline('Can not solve inv')
        #    is_success = False
        #if not is_success:
        #        SI = np.linalg.pinv(S.T) #TODO 普通の疑似逆行列は左からかける前提，今回は右からだが，影響はないか
        #        inv = 'pinv'
        #    # try:
            #     SI = np.linalg.pinv(S) #TODO 普通の疑似逆行列は左からかける前提，今回は右からだが，影響はないか
            #     inv = 'pinv'
            # except:
            #     pm.printline('Can not solve pinv')
            #     SI = I
            #     inv = 'I'
        # SIが計算できない場合，単位行列にする。（TODO これだと分母が１になり，ゲインが１を超え，発散する危険性がある)`
        ...
        diff_ais_jcope_n = np.mean(np.abs(data['AIS-JCOPE_N']))
        logger.record_tabular(f"AIS-JCOPE", mean_diff(z, H@jcope))
        logger.record_tabular(f"Kalman-JCOPE", mean_diff(H@x, H@jcope))
        logger.record_tabular(f"AIS-Kalman", mean_diff(z, H@x))
        logger.dump_tabular()
        ...
        with open(f'{save_dir}/saverK{year}{month:02}{day:02}{t:02}-N{_N}.pkl', 'wb') as f:
            pickle.dump(K, f)
        with open(f'{save_dir}/saverJCOPECur{year}{month:02}{day:02}{t:02}-N{_N}.pkl', 'wb') as f:
            pickle.dump(jcor, f)
        with open(f'{save_dir}/saverXCur{year}{month:02}{day:02}{t:02}-N{_N}.pkl', 'wb') as f:
            pickle.dump(H@x, f)
