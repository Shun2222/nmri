...
import utils.printManager as pm
...
def kalman_filter():
    for Q_value in Q_values:
        ...
        for r in range(1):
            ...
            print(f'ais shape: {z.shape}')
            nonZero = z!=0.0
            tf = (~np.isnan(z)) & (nonZero)
            print(f'true num: {np.sum(tf)}')
            ...
            for day in range(1, n_day):
                ...
                for t in tqdm.tqdm(range(0,24)):
                    ...
                    for ais_cur, ais_lambda, ais_phi, target_name in zip(ais_curs, ais_lambdas, ais_phis, ['v1', 'v2']):
                        ...
                        pkl.dump(x, open(f'{save_dir}/saverX{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        pkl.dump(z, open(f'{save_dir}/saverZ{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        pkl.dump(jcope, open(f'{save_dir}/saverJCOPE{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        #pkl.dump(P, open(f'{save_dir}/saverP{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        #pkl.dump(R, open(f'{save_dir}/saverR{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        #pkl.dump(F, open(f'{save_dir}/saverF{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        pkl.dump(H, open(f'{save_dir}/saverH{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        #pkl.dump(K, open(f'{save_dir}/saverK{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        pkl.dump(jcor, open(f'{save_dir}/saverJCOPECur{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
                        pkl.dump(H@x[tf2], open(f'{save_dir}/saverXCur{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
