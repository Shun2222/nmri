...
class Ais4ToCur:
        ...
        for inFile in inFiles:
            with open(inFile, "r") as sr:
                ...
                while True:
                    ...
                    LatIdx = int(ss[2])
                    LonIdx = int(ss[3])
                    if not Settei.krp.is_in_kurosio(LatIdx/36, LonIdx/36):
                        continue
    ...
    def load_ais4s(self, dt, filepath):
            ...
            for latidx in range(a.LatIdx - Settei.LatRange, a.LatIdx + Settei.LatRange + 1):
                for lonidx in range(a.LonIdx - Settei.LonRange, a.LonIdx + Settei.LonRange + 1):
                    if Settei.LatIdxMin <= latidx <= Settei.LatIdxMax and \
                        ...
                        curs.append(Cur(a.MMSI, dtidx, latidx, lonidx, curN, curE))
...
class ShipVarManager:
    ...
    def update(self, kalman_x, is_target):
        for cur in self.Curs:
            grid0, grid1 = latlon_to_mesh(cur.LatIdx, cur.LonIdx)
            idx = kurosio_index[grid0][grid1]
            if not is_target[idx]:
                continue
            
            targets = np.where(is_target)[0]
            idxN = np.where(targets==idx)[0][0]
            idxE = idxN + len(idxN)

            if not cur.MMSI in self.errors:
                self.errors[cur.MMSI] = []
            error = kalman_x[idxN] - cur.CurN
            self.errors[cur.MMSI].append(error)
            error = kalman_x[idxE] - cur.CurE
            self.errors[cur.MMSI].append(error)
    
        
        for mmsi in self.errors.keys():
            sum_error = np.sum(self.errors[mmsi]) / len(self.errors[mmsi])
            np_error = np.array(self.errors[mmsi])
            var = np.sum(np_error - sum_error)**2 / (len(self.erros[mmsi])-1)
            self.vars[mmsi] = vars
    ...
    def save_info(self, filename):
        pkl.dump(self.errors, open(osp.join(self.out_folder, 'error'+ filename + '.pkl'), 'wb'))
        pkl.dump(self.vars, open(osp.join(self.out_folder, 'var' + filename + '.pkl'), 'wb'))
...
if __name__ == "__main__":
    ...
    if False:
        inFiles = [r'E:\shunsukeE\data\ais\150901-6log\log\japan_20150901100000-20150901115959.ais4']
        outFolder = r'data/ais_test'
        atc = Ais4ToCur()
        atc.load_ais4(inFiles, outFolder)
        dtidx = atc.ais4s[0].DtIdx
        atc.calc_cur(dtidx)
        svm.print_info()
    ...
    if True:
        ...
        out_folder = r'E:/shunsukeE/data/ais/1509-ais4s-pkls'
        ais_keys = ['cur1', 'cur2', 'lambda1', 'lambda2', 'phi1', 'phi2', 'n', 'e']
        al = AISLoader(year, month, out_folder, pkl_path='data/ais_test')
        al.set_keys(ais_keys)
