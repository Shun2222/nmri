...
save_dir = r"E:\shunsukeE\result\kalman-all-Q001"
...
for r in range(1):
    ...
    cur1 = kurosio_filter(ais_cur1, nan_map_pooled)# 時刻0のais data
    ...
    TF = np.array([True for _ in range(len(cur1))])
    ...
    def get_z(ais_cur1, ais_cur2, dtidx, default=None):
        ...
        if len(ais_cur1)==0 or len(ais_cur2)==0: 
            ...
            nan_data = [np.nan for _ in range(_2N0)] + [1]
            return np.array(nan_data) 

        ais_cur1_dt = kurosio_filter(ais_cur1, nan_map_pooled)[TF] # 時刻0のais data
        ais_cur2_dt = kurosio_filter(ais_cur2, nan_map_pooled)[TF] # 時刻0のais data

        ais_cur12_dt = np.concatenate([ais_cur1_dt, ais_cur2_dt])
        ais_cur12_dt = np.concatenate([ais_cur12_dt, [1]])
        ais_cur12_dt = ais_cur12_dt.reshape(_2N0+1, 1)
    ...
    def get_x(jcope_n, jcope_e, dtidx, default=np.nan):
        ...
        jcope_n_dt = kurosio_filter(jcope_n[dtidx], nan_map_pooled)[TF] # 時刻0のjcope data
        jcope_e_dt = kurosio_filter(jcope_e[dtidx], nan_map_pooled)[TF] # 時刻0のjcope data
    ...
    ## H関数
    lambda1 = kurosio_filter(ais_lambda1, nan_map_pooled)[TF]
    lambda2 = kurosio_filter(ais_lambda2, nan_map_pooled)[TF]
    lambda1 = lambda1[tf_ravel_hl]
    lambda2 = lambda2[tf_ravel_hl]
    lambda1[lambda1==0] = 0.001
    lambda2[lambda2==0] = 0.001

    phi1 = kurosio_filter(ais_phi1, nan_map_pooled)[TF]
    phi2 = kurosio_filter(ais_phi2, nan_map_pooled)[TF]
    ...
    ## 分散共分散行列
    Q = np.eye(_2N0+1)*0.01 # システム誤差(jcope)
    Q[-1][-1] = 0.0
    ...
    for day in range(1, n_day):
        ...
        for t in tqdm.tqdm(range(0,24)):
            ...
            #pkl.dump(F, open(f'{save_dir}/saverF{year}{month:02}{day:02}{t:02}-{r}.pkl', 'wb':_N0))
