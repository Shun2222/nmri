...
class ShipVarManager:
    ...
    def update(self, kalman_x, is_target):
        for cur in self.Curs:
            grid0, grid1 = latlon_to_mesh(cur.LatIdx/36, cur.LonIdx/36)
            ...
            print(f'Updated idx:{idx} idxN:{idxN}, idxE{idxE}')

            if not cur.MMSI in self.errors:
                self.errors[cur.MMSI] = []
            error = kalman_x[idxN] - cur.curN # 誤差の計算
        ...
        for mmsi in self.errors.keys():
            ...
            #var = np.sum(np_error - sum_error)**2 / (len(self.errors[mmsi])-1) # 分散の計算
            #self.vars[mmsi] = var 
...
if __name__ == "__main__":
    ...
    if True:
        ...
        for i in range(7, 8):
            print(f'day: {i}')
            #al.load_test(i)
            al.load_ais_day(i, use_pkl=True)
            print('\n')
    ...
    if False:
        ...
        day = 1
        data = al.load_ais_day(day)# AIS4ToCur classでcsvファイルで保存されたファイルのうちset_keysで設定したファイルを読み込み、辞書型で返す（例：data['cur1'] = 固有値方向の偏流１のデータ）
