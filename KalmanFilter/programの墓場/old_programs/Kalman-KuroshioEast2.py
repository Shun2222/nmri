...
data = kurosio_filter(ais_cur1[dtidx][0], nan_map)
n = _N = len(data)
m = _M = 1
_NM = _N * _M
_2NM = 2 * _NM
...
def F_mat(jcope_n, jcope_e, dtidx):
    ...
    for i in range(_2NM):
        ...
        F[i][-1] = residual if residual==residual else 0.0
    F[_2NM][-1] = 1.0
    #print(f'F:{F}F')
    return F
...
## 分散共分散行列
#kf.Q = np.eye(_2NM+1)*0.001 # システム誤差(jcope)
...
#for day in range(23, 23+1):
    ...
    for t in tqdm.tqdm(range(1,24)):
        ...
        debug = True    
        if debug:
            with open(f'{save_dir}/saverP{year}{month:02}{t-1:02}.pkl', 'wb') as f:
                pickle.dump(P, f)
            with open(f'{save_dir}/saverR{year}{month:02}{t-1:02}.pkl', 'wb') as f:
                pickle.dump(R, f)
            with open(f'{save_dir}/saverF{year}{month:02}{t-1:02}.pkl', 'wb') as f:
                pickle.dump(F, f)
            with open(f'{save_dir}/saverH{year}{month:02}{t-1:02}.pkl', 'wb') as f:
                pickle.dump(H, f)
