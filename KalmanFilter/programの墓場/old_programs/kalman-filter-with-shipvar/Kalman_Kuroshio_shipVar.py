...
save_dir = r"E:\shunsukeE\result\kalman-shipVar-time-test3"
...
# data = al.load_ais_dtidx(dtidx, keys=['cur1'])
# ais_cur1 = data['cur1']
...
for r in range(len(_Targets)//_Nslice+1):
    ...
    dt = datetime.datetime(dt_year, dt_month, day, 0, 0, 0)
    dtidx = date_to_dtidx(base_dt, dt)
    cur1 = kurosio_filter(ais_cur1[dtidx], nan_map)# 時刻0のais data
    ...
    def get_z(ais_cur1, ais_cur2, dtidx, default=None):
        ...
        ais_cur1_dt = kurosio_filter(ais_cur1[dtidx], nan_map)[TF] # 時刻0のais data
        ais_cur2_dt = kurosio_filter(ais_cur2[dtidx], nan_map)[TF] # 時刻0のais data
    ...
    ## H関数
    lambda1 = kurosio_filter(ais_lambda1[dtidx], nan_map)[TF]
    lambda2 = kurosio_filter(ais_lambda2[dtidx], nan_map)[TF]
    lambda1 = lambda1[tf_ravel_hl]
    lambda2 = lambda2[tf_ravel_hl]
    lambda1[lambda1==0] = 0.001
    lambda2[lambda2==0] = 0.001

    phi1 = kurosio_filter(ais_phi1[dtidx], nan_map)[TF]
    phi2 = kurosio_filter(ais_phi2[dtidx], nan_map)[TF]
