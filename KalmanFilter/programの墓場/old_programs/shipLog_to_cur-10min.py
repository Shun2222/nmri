...
def divide_nmea(log_data):
    ...
    for t in time_utc:
        ...
        time_idx2.append(int(idx2.days*24*600 + idx2.seconds / (600)))
...
for target_ship in target_ships.keys():
    ...
    for day in range(1, n_day+1):
        ...
        f = f'{target_ships[target_ship]}FileOut{year}{month:02}{day:02}.slog1'
        f_path = osp.join(path_log, f)
        try:
    ...
    for i in range(len(grid_cur_minute)):
        # 保存
        grid_cur_m = grid_cur_minute[i]
        grid_cur_m = grid_cur_m.sort_values('DtIdx_Minute')
        grid_cur_m = grid_cur_m.reset_index(drop=True)

        day = path_name[i][-8:-6]

        save_path = osp.join(path_logs[i], f"cur_10minutes{dt_year}{dt_month:02}{day}.csv")
