...
def insecs_plot(data_info, ss, ss2, cur, line_plot=True, save=False, savepath='./'):
    ...
    for cur_mmsi in cur[1]:
        ...
        cur_mmsi = [float(c.split(':')[1]) for c in cur_mmsi]
        cur_mmsi_datas.append(cur_mmsi)
    ...
    for ax in [ax0, ax1, ax2]:
        ...
        if np.abs(cur_value2[0]) + np.abs(cur_value2[1]) < 10:
            ax.scatter(cur_value2[0], cur_value2[1], color='orange', alpha=1.0, marker="*", label="MinDeltaLine")
        ...
        if len(cur_value4)!=0:
            if np.abs(cur_value4[0]) + np.abs(cur_value4[1]) < 10:
                ax.scatter(cur_value4[0], cur_value4[1], color='blue', alpha=1.0, marker="*", label="LambdaLSM")
        for cur_mmsi in cur_mmsi_datas:
            print(cur_mmsi)
            if np.abs(cur_mmsi[1]) + np.abs(cur_mmsi[2]) < 10:
                ax.scatter(cur_mmsi[1], cur_mmsi[2], color='yellow', alpha=1.0, marker="*", label="MMSI:"+str(cur_mmsi[0]))
...
if __name__ == "__main__":
    ...
    for data_info, ss, ss2, cur in zip(data_infos, sss, ss2s, curs):
        insecs_plot(data_info, ss, ss2, cur, line_plot=False, save=True, savepath='images/test8')
