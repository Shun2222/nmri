...
for Q_value in Q_values:
    ...
    save_dir = fr"E:\shunsukeE\result\kalman-entire-pooled6-point-Q{Q_value}" # データの保存先
    ...
    ais_path = r'E:/shunsukeE/data/ais/1509-ais4s-pkls-pooled6-entire'
    ...
    for r in range(1):
        ...
        def H_mat(phi2):
            ...
            H[-1, -1] = 1.0
            length = H.shape[0] # これを入れると配列の長さを計算して内部で保持してる？
            return H
        ...
        pkl.dump((H@x[tf2]).toarray(), open(f'{save_dir}/saverXCur{year}{month:02}{day:02}00-{r}.pkl', 'wb'))
        ...
        for day in range(1, n_day):
            ...
            for t in tqdm.tqdm(range(0,24)):
                ...
                for ais_cur, ais_lambda, ais_phi, target_name in zip(ais_curs, ais_lambdas, ais_phis, ['v1', 'v2']):
                    ...
                    _NM = _N * _M
                    _2NM = 2 * _NM
                    I = sps.eye(_2NM+1, dtype=np.float32) 
                    print(f'New _N = {_N}')
                    ...
                    P_np = P.toarray()
                    Ptf = sps.csr_matrix(P_np[tf2.ravel()].T[tf2.ravel()])
                    Ptf = I_KH @ Ptf @ I_KH.T + K @ R @ K.T
                    Ptf_np = Ptf.toarray()
                    ...
                    pkl.dump((H@x[tf2]).toarray(), open(f'{save_dir}/saverXCur{year}{month:02}{day:02}{t:02}-{target_name}.pkl', 'wb'))
