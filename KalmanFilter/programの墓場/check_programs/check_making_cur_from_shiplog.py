...
def cur_nmea(gga, vbw, hdt, vtg):
    ...
    time_set_vbw = set(vbw["DtIdx_Minute"])
    count = 0
    count_v = 0
    all_count = 0
    for time in time_set:
        all_count += 1 
        if time in time_set_vtg:
            count += 1
            if time in time_set_vbw:
                count_v += 1
    print(100*count/all_count)
    print(100*count_v/all_count)
    print(count)
    print(count_v)
    input()
    for time in time_set:
...
for target_ship in target_ships.keys():
    ...
    for day in target_days:
        f_path = osp.join(path2, f'{target_ships[target_ship]}FileOut{year}{month:02}{day:02}.slog1')
        try:
