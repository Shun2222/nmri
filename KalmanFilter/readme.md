...
- [Introduction](#introduction)
  - [軸について](#軸について)
- [AISデータによる偏流の計算式](#aisデータによる偏流の計算式)
  - [船単位のデータで偏流計算](#船単位のデータで偏流計算)
    - [固有値方向の対地ベクトルの寄与（VogをNE方向でなく，固有値方向に変換）](#固有値方向の対地ベクトルの寄与vogをne方向でなく固有値方向に変換)
  - [全船の偏流を重みづけで計算](#全船の偏流を重みづけで計算)
  - [実装内容](#実装内容)
  - [固有値の計算](#固有値の計算)
- [船のログによる偏流値の計算式](#船のログによる偏流値の計算式)

# Introduction
KalmanFilterフォルダ内のプログラムの計算式の詳細．
論文とすべて同じ．
