...
class Settei:
    ...
    def add_map(DtIdx, LatIdx, LonIdx, item, Value):
        ...
        if item==5:
            lineCount = Settei.Map[DtIdx % Settei.h][LatIdx - Settei.LatIdxMin][LonIdx - Settei.LonIdxMin][7]
            if lineCount!=0 or lineCount<Value[2]:
                Settei.Map[DtIdx % Settei.h][LatIdx - Settei.LatIdxMin][LonIdx - Settei.LonIdxMin][5] = Value[0]
                Settei.Map[DtIdx % Settei.h][LatIdx - Settei.LatIdxMin][LonIdx - Settei.LonIdxMin][6] = Value[1]
                Settei.Map[DtIdx % Settei.h][LatIdx - Settei.LatIdxMin][LonIdx - Settei.LonIdxMin][7] = Value[2]
        else:
    ...
    def map_to_csv_and_clear(DtIdx):
        ...
        with    open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}Cur1_2.csv", "w") as sw_cur1_2, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}Cur2_2.csv", "w") as sw_cur2_2, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}N2.csv", "w") as sw_n2, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}E2.csv", "w") as sw_e2, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}D2.csv", "w") as sw_d, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}Lambda1_2.csv", "w") as sw_lambda1, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}Lambda2_2.csv", "w") as sw_lambda2, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}Phi1_2.csv", "w") as sw_phi1, \
                open(f"{Settei.OutFolder}/AisCurr{dt_format.strftime('%Y%m%d%H')}Phi2_2.csv", "w") as sw_phi2:
            ...
            for LatIdx in range(Settei.LatIdxMax, Settei.LatIdxMin - 1, -1):
                ...
                for LonIdx in range(Settei.LonIdxMin, Settei.LonIdxMax + 1):
                    ...
                    N = Settei.Map[DtIdx % Settei.h][LatIdx - Settei.LatIdxMin][LonIdx - Settei.LonIdxMin][5]
                    E = Settei.Map[DtIdx % Settei.h][LatIdx - Settei.LatIdxMin][LonIdx - Settei.LonIdxMin][6]
                    D = A11 * A22 - A12 * A12
                    ...
                    if D >= 1:
                        ...
                        Cur1_2 = (N * math.cos(Phi1) + E * math.sin(Phi1))
                        Cur2_2 = (N * math.cos(Phi2) + E * math.sin(Phi2))
                        s_cur1_2 += f"{Cur1_2:.2f},"
                        s_cur2_2 += f"{Cur2_2:.2f},"
                        s_d += f"{D:.2f},"
                        s_n += f"{N:.2f},"
                        s_e += f"{E:.2f},"
                ...
                sw_cur1_2.write(s_cur1_2[:-1] + "\n")
                sw_cur2_2.write(s_cur2_2[:-1] + "\n")
                sw_d.write(s_d[:-1] + "\n")
                sw_n2.write(s_n[:-1] + "\n")
                sw_e2.write(s_e[:-1] + "\n")
...
class Ais4ToCur:
        ...
        for inFile in inFiles:
            with open(inFile, "r") as sr:
                ...
                while True:
                    ...
                    N = float(ss[7])
                    E = float(ss[8])
                    ais4s.append(ais4(MMSI, DtIdx, LatIdx, LonIdx, ThetaDeg, F, LineCount, N, E))
                    key = f'({LatIdx},{LonIdx})'
                    if key in map_ndata:
                        map_ndata[key] += 1
                    else:
    ...
    def calc_cur(self, target_dtidx):
            ...
            for latidx in range(a.LatIdx - Settei.LatRange, a.LatIdx + Settei.LatRange + 1):
                for lonidx in range(a.LonIdx - Settei.LonRange, a.LonIdx + Settei.LonRange + 1):
                    if Settei.LatIdxMin <= latidx <= Settei.LatIdxMax and \
                        ...
                        Settei.add_map(dtidx, latidx, lonidx, 5, [N, E, lineCount])
        ...
        svm.set_curs(curs)
        keys = ['Cur1_2', 'Cur2_2', 'Lambda1_2', 'Lambda2_2', 'Phi1_2', 'Phi2_2', 'N2', 'E2', 'D2']
        keys2 = ['cur1_2', 'cur2_2', 'lambda1_2', 'lambda2_2', 'phi1_2', 'phi2_2', 'n2', 'e2', 'd2']
        saved_filenames = {}
...
class ais4:
    def __init__(self, MMSI, DtIdx, LatIdx, LonIdx, ThetaDeg, Flow, LineCount, N, E):
        self.MMSI = MMSI
        self.DtIdx = DtIdx
        self.LatIdx = LatIdx
        self.LonIdx = LonIdx
...
# AISのファイルパスの読み取り
    def __init__(self, year, month, out_folder, pkl_path=None):
        ...
        self.pathes = None
        self.keys = ['cur1_2', 'cur2_2', 'lambda1_2', 'lambda2_2', \
                     'phi1_2', 'phi2_2', 'd', 'n2', 'e2']
        self.out_folder = out_folder
    ...
    def get_ais4(self, dtidx):
        ...
        if hour%2 == 0:
            if hour <= 14:
                i = int(hour/2) + 5
                filename = fr'japan_2015{month:02}{day:02}{i*2-2:02}0000-2015{month:02}{day:02}{i*2-1:02}5959{ext}'
                file_path.append(fr"{path}\1509{day:02}-{i}log\log\{filename}")
            else:
                i = int(hour/2) - 7
                
                if day+1==8 and (i==2 or i==3):
                    filename = fr'japan_2015{month:02}{day+1:02}{i*2-2:02}0000-2015{month:02}{day+1:02}{i*2-2:02}5959{ext}'
                    file_path.append(fr"{path}\15{month:02}{day+1:02}-{i}-1log\log\{filename}")
                    filename = fr'japan_2015{month:02}{day+1:02}{i*2-1:02}0000-2015{month:02}{day+1:02}{i*2-1:02}5959{ext}'
                    file_path.append(fr"{path}\15{month:02}{day+1:02}-{i}-2log\log\{filename}")
                else:
                    filename = fr'japan_2015{month:02}{day+1:02}{i*2-2:02}0000-2015{month:02}{day+1:02}{i*2-1:02}5959{ext}'
            ...
            if hour < 15:
                ...
                filename = fr'japan_2015{month:02}{day:02}{(i+1)*2-2:02}0000-2015{month:02}{day:02}{(i+1)*2-1:02}5959{ext}'
                ...
                if day+1==8 and (i+1==2 or i+1==3):
                    filename = fr'japan_2015{month:02}{day+1:02}{(i+1)*2-2:02}0000-2015{month:02}{day+1:02}{(i+1)*2-2:02}5959{ext}'
                    file_path.append(fr"{path}\15{month:02}{day+1:02}-{i+1}-1log\log\{filename}")
                    filename = fr'japan_2015{month:02}{day+1:02}{(i+1)*2-1:02}0000-2015{month:02}{day+1:02}{(i+1)*2-1:02}5959{ext}'
                    file_path.append(fr"{path}\15{month:02}{day+1:02}-{i+1}-2log\log\{filename}")
                else:
                    filename = fr'japan_2015{month:02}{day+1:02}{(i+1)*2-2:02}0000-2015{month:02}{day+1:02}{(i+1)*2-1:02}5959{ext}'
                    file_path.append(fr"{path}\15{month:02}{day+1:02}-{i+1}log\log\{filename}")                          
            else:
                filename = fr'japan_2015{month:02}{day:02}{12*2-2:02}0000-2015{month:02}{day:02}{12*2-1:02}5959{ext}'
                file_path.append(fr"{path}\15{month:02}{day:02}-{12}log\log\{filename}")
                filename = fr'japan_2015{month:02}{day+1:02}{1*2-2:02}0000-2015{month:02}{day+1:02}{1*2-1:02}5959{ext}'
...
if __name__ == "__main__":
    ...
    if True:
        ...
        ais_keys = ['cur1', 'cur2', 'cur1_2', 'cur2_2', 'lambda1', 'lambda2', 'phi1', 'phi2', 'n', 'e']
